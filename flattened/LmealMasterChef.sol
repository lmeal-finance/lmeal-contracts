...
// File: contracts\LmealToken.sol

pragma solidity 0.6.12;


// LmealToken with Governance.
contract LmealToken is BEP20('Lmeal Token', 'LMEAL') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "LMEAL::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "LMEAL::delegateBySig: invalid nonce");
        require(now <= expiry, "LMEAL::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "LMEAL::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying LMEALs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "LMEAL::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts\LmealSyrupBar.sol

pragma solidity 0.6.12;



// LmealSyrupBar with Governance.
contract LmealSyrupBar is BEP20('Lmeal SyrupBar Token', 'LMEAL SYRUP') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    function safeLmealTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 lmealBal = lmeal.balanceOf(address(this));
        if (_amount > lmealBal) {
            lmeal.transfer(_to, lmealBal);
        } else {
            lmeal.transfer(_to, _amount);
        }
    }
...
// File: contracts\LmealMasterChef.sol
